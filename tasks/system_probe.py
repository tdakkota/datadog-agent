...
def build_network_ebpf_compile_file(ctx, parallel_build, build_dir, p, debug, network_prebuilt_dir, network_flags):
    ...
    if not debug:
        ...
        return ctx.run(CLANG_CMD.format(flags=" ".join(network_flags), bc_file=bc_file, c_file=src_file), asynchronous=parallel_build)
    else:
        debug_bc_file = os.path.join(build_dir, "{}-debug.bc".format(p))
        return ctx.run(CLANG_CMD.format(flags=" ".join(network_flags + ["-DDEBUG=1"]), bc_file=debug_bc_file, c_file=src_file), asynchronous=parallel_build)
...
def build_network_ebpf_link_file(ctx, parallel_build, build_dir, p, debug, network_flags):
    if not debug:
        ...
        return ctx.run(LLC_CMD.format(flags=" ".join(network_flags), bc_file=bc_file, obj_file=obj_file), asynchronous=parallel_build)
    else:
        debug_bc_file = os.path.join(build_dir, "{}-debug.bc".format(p))
        debug_obj_file = os.path.join(build_dir, "{}-debug.o".format(p))
        return ctx.run(LLC_CMD.format(flags=" ".join(network_flags), bc_file=debug_bc_file, obj_file=debug_obj_file), asynchronous=parallel_build)
...
def build_network_ebpf_files(ctx, build_dir, parallel_build=True):
    ...
    for p, debug in flavor:
        promises.append(build_network_ebpf_compile_file(ctx, parallel_build, build_dir, p, debug, network_prebuilt_dir, network_flags))
...
def build_security_ebpf_files(ctx, build_dir, parallel_build=True):
    ...
    promises.append(ctx.run(
        LLC_CMD.format(
            ...
            bc_file=security_bc_file,
            obj_file=security_agent_obj_file
        ),
        asynchronous=parallel_build
    ))
