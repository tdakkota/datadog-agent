...
func TestBatchStrategySendsPayloadWhenBufferIsOutdated(t *testing.T) {
	...
	clk := clock.NewMock()
	strategy := newBatchStrategyWithClock(LineSerializer, timerInterval, 0, 100, 100, "test", clk)
	done := make(chan bool)
	go func() {
...
func TestBatchStrategySendsPayloadWhenClosingInput(t *testing.T) {
	...
	go func() {
		newBatchStrategyWithClock(LineSerializer, 100*time.Millisecond, 0, 2, 2, "test", clk).Send(input, output, success)
		close(done)
	}()
