...
func newCreditCardsObfuscator(useLuhn bool) *ccObfuscator {
	...
	pb.SetMetaHook(cco.CheckTag)
	return cco
}

// CheckTag checks the tag with the given key and val and returns the final
...
func (cco *ccObfuscator) CheckTag(k, v string) (newval string) {
	switch k {
	case "_sample_rate",
